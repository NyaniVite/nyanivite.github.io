...
const api = new $vite_vitejs.ViteAPI(wsProvider, async () => {
  ...
  for (let i = 0; i < pairCount[0]; i++) {
    ...
    document.getElementById('swapPage_pairs_page').innerHTML += `
      <li id="${i}" class="is-clickable is-unselectable" style="background-color: #6707ac !important;">
  ...
  document.getElementById('swapPage_pairs_page').addEventListener('click', async function (e) {
    ...
    if (target.tagName === 'LI') {
      ...
      document.getElementById('swapPage_inputAmount').oninput = async () => {
        ...
        document.getElementById('swapPage_outputAmount').value = `${Big(rate).div(`1e${pairTokens[1]['decimals']}`).toFixed()}`
      }
      ...
      document.getElementById('swapPage_outputAmount').oninput = async () => {
        ...
        document.getElementById('swapPage_inputAmount').value = `${Big(rate).div(`1e${pairTokens[0]['decimals']}`).toFixed()}`
      }
      ...
      document.getElementById('swapPage_swap').onclick = async () => {
        ...
        const block = await $vite_vitejs.accountBlock.createAccountBlock("callContract", {
          ...
          methodName: "swap",
          params: [ pairTokens[1]["tokenId"], "1", "100000" ],
      ...
      document.getElementById('lqPage_wrapTokens').onchange = async () => {
        if (typeof viteConnect.connectedAddress === 'undefined') return
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.getElementById('tabs_stake').onclick = function () {
    api.nya.toast('success', 'Coming soon...')
